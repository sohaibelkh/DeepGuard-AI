...
class ECG_CNN_LSTM(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = nn.Conv1d(in_channels, cnn_channels[0], kernel_size=7, padding=3)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x shape: (batch, channels, signal_length)
...
class HybridModel:
    ...
    def predict(self, signal: np.ndarray) -> tuple[str, float, dict[str, float], float]:
        ...
        return self.classes[idx], float(probs[idx]), class_probs, 0.88
